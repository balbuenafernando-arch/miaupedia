...
export async function loadPrivateData(userId: string) {
  ...
  const cats:CatProfile[]=(catRows||[]).map(row=>({id:row.id,name:row.nombre,age:row.edad||0,sex:row.sexo,breed:row.raza||"Sin raza definida",sterilized:row.esterilizado,birthDate:row.fecha_nacimiento||"",weight:Number(row.peso||0),photo:row.foto||"",food:row.cat_preferences?.[0]?.alimentos||[],litter:row.cat_preferences?.[0]?.arenas||[],toys:row.cat_preferences?.[0]?.juguetes||[]}));
  ...
  const userReviews:UserReview[]=(reviewRows||[]).map(r=>({id:r.id,placeId:0,rating:r.puntuacion,comment:r.comentario,status:r.estado,createdAt:r.created_at,helpful:r.helpful_count,unhelpful:r.unhelpful_count,reported:false}));
  return {cats,reminders,userReviews,follows:(followRows||[]).map((_r,i)=>i)};
}

export async function saveCat(userId:string,cat:CatProfile){const db=createClient();if(!db)return;const {error}=await db.from("cats").upsert({id:cat.id,user_id:userId,nombre:cat.name,foto:cat.photo||null,sexo:cat.sex,edad:cat.age,fecha_nacimiento:cat.birthDate||null,peso:cat.weight||null,raza:cat.breed,esterilizado:cat.sterilized});if(error)throw error;await db.from("cat_preferences").upsert({cat_id:cat.id,alimentos:cat.food,arenas:cat.litter,juguetes:cat.toys},{onConflict:"cat_id"})}
...
export async function saveReview(userId:string,r:UserReview,place:PlaceReference){const db=createClient();if(!db)return;const base={id:r.id,user_id:userId,puntuacion:r.rating,comentario:r.comment,estado:r.status,helpful_count:r.helpful,unhelpful_count:r.unhelpful};const result=place.kind==="business"?await db.from("reviews").upsert({...base,business_id:place.id}):await db.from("reviews").upsert({...base,product_id:place.id});if(result.error)throw result.error}
